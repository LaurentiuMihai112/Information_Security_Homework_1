...
KeyManager, addr = s.accept()
print('Got connection from', addr)
nodeB, addr = s.accept()
print('Got connection from', addr)

KeyManager.send('Please send encryption key'.encode())
encrypted_key = KeyManager.recv(512)
...
KeyManager.close()
while True:
    operation_mode = input("Please provide operation mode :")
    print(operation_mode)
    operation_mode = operation_mode[:3]
    print(operation_mode)
    if operation_mode.startswith("ECB") or operation_mode.startswith("OFB"):
        break
    else:
        print("Operation mode must be ECB or OFB")
nodeB.send(operation_mode.encode('ascii'))
nodeB.send(encrypted_key)
print("Encrypted Key is " + str(encrypted_key))
print("Decrypting...")
enc_key = decrypt_block(encrypted_key, second_key)
print("Decrypted key is " + enc_key.decode('ascii'))
communication_msg = nodeB.recv(512).decode('ascii')
print(communication_msg)
if communication_msg == "Let's communicate":
    with open(file_path) as f:
        file_content = f.read(512)
    print(file_content)
    encrypted_text = encrypt(file_content.encode('ascii'), operation_mode, enc_key, initialization_vector)
    print("Encrypted text is:" + str(encrypted_text))
    nodeB.send(encrypted_text)
nodeB.close()
s.close()
